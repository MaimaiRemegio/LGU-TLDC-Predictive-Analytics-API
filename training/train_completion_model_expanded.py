"""
Train the Random Forest completion model using the expanded course coverage dataset.

EXPANDED VERSION: Supports all 21 forecasting courses + Bread and Pastry NC II (22 total).
...
# Use the expanded training dataset
TRAINING_DATA_PATH = PROJECT_ROOT / "datasets" / "historical_training_expanded.csv"

# Save expanded model to new location (don't overwrite production model yet)
EXPANDED_MODEL_PATH = PROJECT_ROOT / "trained_models" / "completion_model_expanded.pkl"
EXPANDED_ENCODERS_PATH = PROJECT_ROOT / "trained_models" / "completion_encoders_expanded.pkl"
EXPANDED_METRICS_PATH = PROJECT_ROOT / "trained_models" / "completion_model_metrics_expanded.json"
EXPANDED_FEATURE_IMPORTANCE_PATH = PROJECT_ROOT / "trained_models" / "completion_feature_importance_expanded.json"
EXPANDED_MODEL_METADATA_PATH = PROJECT_ROOT / "trained_models" / "completion_model_metadata_expanded.json"
...
def load_training_dataset() -> pd.DataFrame:
    """Load historical training records from the expanded CSV dataset."""
    
    print("Reading expanded training dataset...")
...
def save_artifacts(
    ...
    EXPANDED_MODEL_PATH.parent.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    save_json(
        feature_importance,
        EXPANDED_FEATURE_IMPORTANCE_PATH,
    )
    
    save_json(
        metadata,
        EXPANDED_MODEL_METADATA_PATH,
    )
    
    model_size_mb = (
        EXPANDED_MODEL_PATH.stat().st_size
        / (1024 * 1024)
    )
    ...
    print(
        f"  Model: {EXPANDED_MODEL_PATH.resolve()}"
    )
    ...
    print(
        f"  Encoders: {EXPANDED_ENCODERS_PATH.resolve()}"
    )
    
    print(
        f"  Metrics: {EXPANDED_METRICS_PATH.resolve()}"
    )
    
    print(
        f"  Feature importance: "
        f"{EXPANDED_FEATURE_IMPORTANCE_PATH.resolve()}"
    )
    
    print(
        f"  Metadata: "
        f"{EXPANDED_MODEL_METADATA_PATH.resolve()}"
    )
...
def print_course_analysis(dataframe: pd.DataFrame, label_encoders: dict) -> None:
    """Print per-course analysis."""
    
    print("\n==============================")
    print("COURSE COVERAGE ANALYSIS")
    print("==============================")
    
    course_dist = dataframe["course_applied"].value_counts().sort_index()
    
    print(f"\nTotal courses: {len(course_dist)}")
    print(f"Total barangays: {dataframe['barangay'].nunique()}")
    
    print(f"\nCourses represented:")
    for i, (course, count) in enumerate(course_dist.items(), 1):
        print(f"  {i:2d}. {course:<55s} {count:4d} records")
...
def main() -> None:
    ...
    print("=" * 70)
    print("EXPANDED COMPLETION MODEL TRAINER")
    print("=" * 70)
    print("Training with full 21-course forecasting coverage + Bread and Pastry NC II")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("SUCCESS: Expanded completion model trained")
    print("=" * 70)
    print("\nNOTE: This model has been saved separately and does NOT overwrite")
    print("      the production model. Review metrics before deployment.")
